...
export const SwipeableItem: React.FC<Props> = ({
  ...
  const itemEmoji = item.emoji || tag?.emoji || "🛒";
    ...
    <>
      ...
      >
        ...
        <motion.div
          ...
          onClick={() => onEdit(item)}
          className={clsx(
            "relative bg-white dark:bg-notion-dark-gray-bg rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 flex items-center justify-between cursor-pointer transition-opacity",
          ...
          <div className="flex items-center gap-1.5 min-w-0">
              ...
              <button
                onClick={(e) => {
                  ...
                  if (isShoppingMode) {
                    ...
                    // In packing mode, only allow toggling if it's bought
                    if (item.isBought) {
            ...
            <div className="min-w-0 flex flex-col">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                >
                  {item.quantity}{" "}
                  {item.presentation ? `x ${item.presentation} ${item.unit || "un"}` : (item.unit || "un")}
                </p>
                {item.locationId && (
